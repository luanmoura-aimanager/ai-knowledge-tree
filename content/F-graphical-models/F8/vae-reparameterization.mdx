...
def compute_elbo(A, b, C, x_batch):
    mu    = x_batch @ A.T                             # (M, d_z)
    sigma = np.exp(b)[np.newaxis]                     # (1, d_z)
    eps   = rng.normal(0, 1, (S_mc, len(x_batch), d_z))
...
for step in range(n_steps):
    ...
    x_batch = x_data[idx]
    mu    = x_batch @ A.T
    sigma = np.exp(b)[np.newaxis]
    eps   = rng.normal(0, 1, (S_mc, batch_size, d_z))
    z     = mu[np.newaxis] + sigma * eps
    x_recon = z @ C.T

    # Finite-difference gradients (to avoid autograd dependency)
    h = 1e-4
    elbo_curr = compute_elbo(A, b, C, x_batch)
    grad_C = np.zeros_like(C)
    for i in range(d_x):
        for j in range(d_z):
            C[i,j] += h
            e_plus = compute_elbo(A, b, C, x_batch)
            C[i,j] -= 2*h
            e_minus = compute_elbo(A, b, C, x_batch)
            C[i,j] += h
...
print(f"\nFinal ELBO: {compute_elbo(A, b, C, x_data):.3f}")
```
