...
The sum-product algorithm on a factor graph computes marginals in a single, unified framework.
...
in $O(n \cdot k^2)$ for $k$-state variables.
...
sum-product decoder for LDPC codes are all instances of sum-product on a factor graph.
