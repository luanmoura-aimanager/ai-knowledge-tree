...
rare words decompose into subword pieces (no OOV). Typical vocabulary: 30K-200K entries.

## Byte-pair encoding (BPE)

BPE is a compression algorithm adapted for tokenization (Sennrich et al., 2016).
...
SentencePiece (Kudo and Richardson, 2018) differs in two ways:
...
2. It offers a **unigram language model** variant alongside BPE. The unigram model
   starts with a large vocabulary and prunes entries, keeping those that minimize:
