...
**Hidden Markov Model** is the canonical probabilistic model for this structure<FN ns="1, 2" />.
...
in $O(TK^2)$ with dynamic programming<FN n="3" />.
...
**Exercise 1.** Using the lesson's parameters, complete the worked example: compute $v_2(\text{NN})$, then the three candidate values inside $v_3(\text{VB})$, and confirm the Viterbi path is DT-NN-VB. Recall $\pi = [0.6, 0.3, 0.1]$, the rows of $A$ as given, and the emission column for "runs" is $B[:, \text{runs}] = [0.05, 0.09, 0.95]$.

<details>
<summary>Solution</summary>

**Step 1.** From initialization, $v_1(\text{DT}) = 0.6 \cdot 0.9 = 0.54$, $v_1(\text{NN}) = 0.3 \cdot 0.05 = 0.015$, $v_1(\text{VB}) = 0.1 \cdot 0.0 = 0$.

**Step 2.** Compute $v_2(\text{NN})$ for "dog" ($B_{\text{NN},\text{dog}} = 0.9$). The transitions into NN are $A_{\text{DT},\text{NN}} = 0.8$, $A_{\text{NN},\text{NN}} = 0.1$, $A_{\text{VB},\text{NN}} = 0.4$:

$$
v_2(\text{NN}) = 0.9 \cdot \max(0.54 \cdot 0.8,\ 0.015 \cdot 0.1,\ 0 \cdot 0.4) = 0.9 \cdot 0.432 = 0.3888.
$$

The argmax is DT, so the backpointer $\psi_2(\text{NN}) = \text{DT}$.

**Step 3.** For $v_3(\text{VB})$ at "runs" ($B_{\text{VB},\text{runs}} = 0.95$), the three candidates use the transition column into VB, $A_{\cdot,\text{VB}} = [0.1, 0.8, 0.1]$. Taking $v_2(\text{NN}) = 0.3888$ as the dominant predecessor (the other $v_2$ entries are far smaller):

$$
v_2(\text{NN}) \cdot A_{\text{NN},\text{VB}} = 0.3888 \cdot 0.8 = 0.31104,
$$

which dominates the DT and VB predecessors. So $v_3(\text{VB}) = 0.95 \cdot 0.31104 \approx 0.2955$, with $\psi_3(\text{VB}) = \text{NN}$.

**Step 4.** Backtrack: the max at $t=3$ is VB, whose backpointer is NN, whose backpointer is DT. The path is DT-NN-VB, the correct tagging of "the dog runs".

</details>

**Exercise 2.** The BIO scheme forbids an $O \to I$ transition (an inside tag cannot start a span). Show that setting $A_{O, I} = 0$ before row normalization makes any tag sequence containing an $O$ immediately followed by an $I$ have joint probability zero under the HMM.

<details>
<summary>Solution</summary>

**Step 1.** The HMM joint factorizes as $p(\mathbf{x}, \mathbf{y}) = \pi_{y_1} \prod_t B_{y_t, x_t} \prod_{t \ge 2} A_{y_{t-1}, y_t}$. Every transition between consecutive tags enters as a multiplicative factor $A_{y_{t-1}, y_t}$.

**Step 2.** Suppose the sequence has $y_{t-1} = O$ and $y_t = I$ at some position $t$. Then the product contains the factor $A_{O, I}$.

**Step 3.** By construction $A_{O, I} = 0$. A single zero factor in a product of nonnegative terms makes the whole product zero, so $p(\mathbf{x}, \mathbf{y}) = 0$ regardless of the other factors.

**Step 4.** Since Viterbi maximizes this joint, it never selects a path through a zeroed transition (any competing path with nonzero probability beats it). The hard BIO constraint is enforced exactly, not just discouraged.

</details>

**Exercise 3 (code).** Run Viterbi on the lesson's HMM but change the emission so "dog" is much more likely a verb than a noun ($B_{\text{NN},\text{dog}} = 0.1$, $B_{\text{VB},\text{dog}} = 0.85$). Confirm the decoded tag for "dog" flips from NN to VB.
...
A  = np.array([[0.1, 0.8, 0.1],
               [0.1, 0.1, 0.8],
               [0.5, 0.4, 0.1]])
# "dog" emission changed: now strongly a verb
B  = np.array([[0.9,  0.05, 0.05],
               [0.01, 0.10, 0.89],   # NN row: dog now only 0.10
               [0.0,  0.85, 0.15]])  # VB row: dog now 0.85
...
tag_names = ["DT", "NN", "VB"]
assert tag_names[path[1]] == "VB"   # "dog" now decoded as a verb
print("Viterbi path:", [tag_names[y] for y in path])
```

With the emission shifted, the noun hypothesis for "dog" loses to the verb hypothesis even though the DT-NN transition is favored, so the decoded middle tag flips to VB.
...
<Footnotes>
  <FNItem n="1">**Jurafsky, D., & Martin, J. H.** (2024). *Speech and Language Processing* (3rd ed. draft). HMMs, POS tagging, and named entity recognition.</FNItem>
  <FNItem n="2">**Manning, C. D., & Schütze, H.** (1999). *Foundations of Statistical Natural Language Processing.* MIT Press. Markov models and tagging.</FNItem>
  <FNItem n="3">**Rabiner, L. R.** (1989). "A tutorial on hidden Markov models and selected applications in speech recognition". *Proceedings of the IEEE*, 77(2), 257-286. The standard reference for the forward, Viterbi, and Baum-Welch algorithms. [doi:10.1109/5.18626](https://doi.org/10.1109/5.18626)</FNItem>
</Footnotes>
