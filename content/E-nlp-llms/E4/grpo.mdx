...
by estimating baselines from a **group** of responses sampled from the current policy.
...
For each group of $G$ responses, the objective mirrors PPO's clipped surrogate:
...
def grpo_step(policy, ref_model, reward_fn, prompts, G=8, beta=0.04, eps=0.2):
    ...
    total_loss = 0.0
    for i, (prompt, responses, rewards) in enumerate(zip(prompts, all_responses, all_rewards)):
        r_tensor = torch.tensor(rewards, dtype=torch.float)
        # Group-relative advantage
        A = (r_tensor - r_tensor.mean()) / (r_tensor.std() + 1e-8)

        for j, (response, adv) in enumerate(zip(responses, A)):
