...
**Exercise 1.** For $d_{\text{model}} = 512$ and $d_{\text{ff}} = 2048$, count the FFN
parameters ($W_1, b_1, W_2, b_2$). Then argue from the pre-norm formula
$\text{Block}(x) = x + \text{FFN}(\text{LN}(x + \text{MHA}(\text{LN}(x))))$ why the block is
shape-preserving.

<details>
<summary>Solution</summary>

**Step 1.** $W_1$ is $d_{\text{ff}} \times d_{\text{model}}$ and $W_2$ is
$d_{\text{model}} \times d_{\text{ff}}$, so the two weight matrices hold
$2 d_{\text{model}} d_{\text{ff}}$ parameters. The biases add $d_{\text{ff}} + d_{\text{model}}$:

$$
2 \cdot 512 \cdot 2048 + 2048 + 512 = 2{,}097{,}152 + 2{,}560 = 2{,}099{,}712.
$$

**Step 2.** Trace the shapes. $\text{LN}(x)$ keeps $(T, d_{\text{model}})$; MHA maps
$(T, d_{\text{model}})$ to $(T, d_{\text{model}})$ through $W^O$; the residual adds two
$(T, d_{\text{model}})$ tensors. The FFN sends $d_{\text{model}}$ up to $d_{\text{ff}}$ and
$W_2$ brings it back to $d_{\text{model}}$, and its residual adds same-shaped tensors. Every
sublayer outputs $(T, d_{\text{model}})$, so the block does too. No operation touches the
sequence axis $T$, so the block is shape-preserving.

</details>

**Exercise 2.** Compute layer norm of $x = (1, 2, 3, 4)$ with $\gamma = 1$, $\beta = 0$,
$\epsilon = 0$ by hand, and confirm the result has zero mean and unit variance.

<details>
<summary>Solution</summary>

**Step 1.** The mean over the four features is $\mu = (1+2+3+4)/4 = 2.5$ and the variance is

$$
\sigma^2 = \tfrac{1}{4}\big[(1.5)^2 + (0.5)^2 + (0.5)^2 + (1.5)^2\big] = \tfrac{1}{4}(2.25 + 0.25 + 0.25 + 2.25) = 1.25,
$$

so $\sigma = \sqrt{1.25} = 1.1180$.

**Step 2.** Normalize each feature as $(x_i - \mu)/\sigma$:

$$
\left( \tfrac{-1.5}{1.1180},\ \tfrac{-0.5}{1.1180},\ \tfrac{0.5}{1.1180},\ \tfrac{1.5}{1.1180} \right) = (-1.3416,\ -0.4472,\ 0.4472,\ 1.3416).
$$

**Step 3.** The four outputs sum to $0$, so the mean is $0$. Their squares are
$1.8 + 0.2 + 0.2 + 1.8 = 4$, and dividing by $4$ gives variance $1$. With $\gamma = 1$ and
$\beta = 0$ the affine step leaves this unchanged.

</details>

**Exercise 3 (code).** Implement layer norm and confirm it produces zero mean and unit
variance per token, and that it is invariant to a constant shift of the input (adding a
constant to every feature of a token leaves the output unchanged).
...
rng = np.random.default_rng(0)
T, d = 4, 16
x = rng.normal(3.0, 2.0, (T, d))          # nonzero mean, nonunit variance
g, b = np.ones(d), np.zeros(d)
y = layer_norm(x, g, b, eps=0.0)

assert np.allclose(y.mean(axis=-1), 0.0, atol=1e-6)   # zero mean per token
assert np.allclose(y.var(axis=-1), 1.0, atol=1e-6)    # unit variance per token

y_shift = layer_norm(x + 7.0, g, b, eps=0.0)          # shift every feature
assert np.allclose(y, y_shift, atol=1e-6)             # subtracting mu cancels it
print("per-token mean 0, variance 1, shift-invariant")
```

Subtracting the per-token mean removes any constant offset before scaling, so layer norm
sees only the shape of each token's feature vector, not its absolute level.
