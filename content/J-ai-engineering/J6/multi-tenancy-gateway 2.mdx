...
- **Data isolation.** Every query, retrieval, and cache lookup must be scoped to the calling
  ...
  tagged and filtered by tenant) or, for stricter needs, a separate database per tenant. A bug here
...
Isolation plus a gateway control plane let one system safely serve many customers. Serving them is
