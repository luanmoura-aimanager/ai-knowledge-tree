...
search, and the central object is the recall-versus-speed tradeoff it exposes.
...
graph rather than buckets, but the same tradeoff governs them; the idea here is the
...
corpus. That curve, recall climbing with comparisons, *is* the vector-database tuning knob.
