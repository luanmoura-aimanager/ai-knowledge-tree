...
rubric ("is this answer helpful and correct? yes/no"). It is fast and cheap enough to
run on every build. But a judge is only useful if it agrees with human judgment, and
...
Once the judge is validated, it becomes a **regression gate**. Keep a fixed evaluation
...
whose pass rate fell far enough below baseline to clear the noise threshold. Evaluation
tells you *whether* the system works; the next lesson improves the part most often
responsible when it does not, the [retrieval pipeline](production-rag).
