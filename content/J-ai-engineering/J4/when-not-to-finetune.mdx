...
options and climbing it first, fine-tuning is the last rung, not the first.
...
- **Retrieval (RAG).** When the gap is *knowledge*, the model does not know your
  ...
  weights. RAG updates the instant your documents change; a fine-tuned model is frozen at
  training time.
...
model's *behavior and style*; it does not reliably inject *facts* (and trying to teach
