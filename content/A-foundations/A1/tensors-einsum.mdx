...
by $k$ integers. Order-0 is a scalar, order-1 a vector, order-2 a matrix. An
...
"down" (or in practice: in two factor arrays) is summed over its full range:
...
mechanical: just differentiate the subscript expression index by index. This
