...
These are explicit formulas: no iteration needed. Putting numbers through them
makes the mechanic concrete. Draw $n = 500$ samples from $\mathrm{Gamma}(\alpha=3,
\beta=2)$, the setting in the code below. The sample mean is $\hat\mu_1 = 1.438$
and the sample variance is $\hat\sigma^2 = 0.670$. The two formulas then read off
the estimates directly: $\hat\alpha = 1.438^2 / 0.670 = 3.09$ and $\hat\beta =
1.438 / 0.670 = 2.15$, against the true $(3, 2)$. No likelihood was written, no
optimizer run; two sample summaries went in and two parameter estimates came out.
...
**Exercise 1.** Derive the method-of-moments estimator for the rate $\lambda$ of
an Exponential distribution, whose density is $f(x) = \lambda e^{-\lambda x}$ for
$x \ge 0$ with $E[X] = 1/\lambda$.

<details>
<summary>Solution</summary>

**Step 1.** There is one parameter, so one moment equation suffices. Set the
first population moment equal to the first sample moment:

$$
\mu_1(\lambda) = E[X] = \frac{1}{\lambda} = \hat\mu_1 = \bar X.
$$
...
The estimator is the reciprocal of the sample mean, which matches the intuition
that a small mean signals a high rate of events.

</details>

**Exercise 2.** A Uniform$(0, \theta)$ distribution has $E[X] = \theta/2$. Derive
the method-of-moments estimator $\hat\theta$, then explain why it can produce an
estimate smaller than some observed data point.

<details>
<summary>Solution</summary>

**Step 1.** With one parameter, match the first moment:
$\mu_1(\theta) = \theta/2 = \bar X$, so $\hat\theta = 2\bar X$.

**Step 2.** The estimator uses only the average. If a single large draw sits near
the true $\theta$ but the rest cluster low, $\bar X$ stays small and $2\bar X$ can
fall below that large draw. Method of moments ignores the constraint that every
observation must be at most $\theta$, a constraint the maximum-likelihood
estimator $\max_i X_i$ respects by construction. Matching moments does not
guarantee a feasible estimate.

</details>

**Exercise 3 (code).** Verify the Exponential estimator from Exercise 1. Draw
$n = 2000$ samples from an Exponential with rate $\lambda = 0.5$, compute
$\hat\lambda = 1/\bar X$, and check it lands near the truth.
...
rng = np.random.default_rng(0)
lam_true = 0.5
# numpy parameterizes Exponential by scale = 1/rate.
data = rng.exponential(scale=1.0 / lam_true, size=2000)

lam_hat = 1.0 / data.mean()
print("lambda_hat:", round(lam_hat, 4), " true:", lam_true)
assert abs(lam_hat - lam_true) < 0.05
```

The estimate sits within $0.05$ of the true rate, confirming that inverting the
sample mean recovers $\lambda$.
