---
title: "Causal representation learning"
pillar: B
subId: B5
lessonId: causal-representation-learning
order: 11
estimatedMinutes: 32
goal: "Connect causal structure to identifiable representation learning via independent causal mechanisms, invariant risk minimization, and sparse mechanism shift."
prerequisites: ["causal-forests"]
---

Deep learning models extract representations from data. But which representations
correspond to *causal factors* of variation? If a model learns the true causal
structure of the world, it should generalize under distribution shift in a way that
purely associational models do not. **Causal representation learning** formalizes
this idea and asks when causal structure can be recovered from data<FN n="1" />.

## The core problem

Standard representation learning finds $f$ such that $z = f(x)$ is useful for
downstream tasks. The learned $z$ may be arbitrarily entangled: it might mix
causal factors with their effects, or correlate features that happen to co-vary
in the training distribution.

Two causal problems this creates:

1. **Distribution shift.** Models trained on $P_{\text{train}}$ fail on $P_{\text{test}}$
   if the shift reflects a causal intervention not seen during training.
2. **Spurious correlations.** Associations that hold in training may break at test
   time (e.g., "grass is green" → "this is a cow" holds at farms but not in zoos).

## The principle of independent causal mechanisms

The **ICM principle** (Peters, Schölkopf et al.) states that causal generative
processes consist of autonomous mechanisms. Each mechanism $p(X_i \mid \mathrm{Pa}(X_i))$
can change independently of the others.

Two consequences:

- **Sparse interventions.** Natural interventions (policy changes, environmental
  shifts) typically change only one or a few mechanisms.
- **Invariance.** The mechanism $p(Y \mid \mathrm{causes}(Y))$ is stable across
  environments even when $p(\mathrm{causes}(Y))$ changes.

## Invariant risk minimization (IRM)

Standard ERM minimizes average loss across all training environments. **IRM**
(Arjovsky et al., 2019) seeks a representation $\Phi(x)$ such that the optimal
classifier on top of $\Phi$ is the *same* in all environments:

$$
\min_\Phi \sum_e \mathcal{R}_e(\Phi) \quad \text{s.t.} \quad \Phi \in \arg\min_w \mathcal{R}_e(w \circ \Phi) \; \forall e.
$$

Features that are causally invariant across environments satisfy the constraint;
spurious correlates do not.

**Practical relaxation (IRMv1):**

$$
\min_\Phi \sum_e \mathcal{R}_e(\Phi) + \lambda \left\|\nabla_{w \mid w=1} \mathcal{R}_e(w \circ \Phi)\right\|^2.
$$

The penalty forces $w = 1$ to be a stationary point in every environment, promoting
invariant features.

## Sparse mechanism shift

The **sparse mechanism shift (SMS)** hypothesis (Schölkopf et al., 2021):
when the distribution shifts from one environment to another, only a sparse subset
of causal mechanisms changes<FN n="1" />. This motivates:

1. **Causal discovery:** identify the SCM from multi-environment data by finding
   the sparsest set of mechanisms that differ across environments.
2. **Domain adaptation:** adapt only the changed mechanisms, reusing the rest.

## Nonlinear ICA and identifiability

Classical ICA (pillar F) recovers independent sources from linear mixtures but is
not identifiable for nonlinear mixtures. **Nonlinear ICA** (Hyvärinen and Morioka;
Khemakhem et al., iVAE) shows that with auxiliary variables $u$ (e.g., environment
labels or time) that modulate the source distributions, the latent causal factors
are identifiable up to element-wise transformations:

$$
x = f(z), \quad p(z \mid u) = \prod_k p_k(z_k \mid u).
$$

This ties representation learning directly to causal identifiability: auxiliary
information (environment, context) makes the latent causal factors recoverable.

```mermaid
graph LR
  Z["latent causal<br/>factors z"] -->|nonlinear mixing| X["observations x"]
  X -->|many environments| R["recover identifiable z"]
  R --> I["invariant causal<br/>representation"]
```

## Implementation

```python-static
import numpy as np
from sklearn.linear_model import LinearRegression

rng = np.random.default_rng(0)

# Simulate two environments with sparse mechanism shift
# SCM: X -> Y, env shifts p(X) but not p(Y|X)
def generate_env(n, beta_xy, beta_x_mean, noise_scale=1.0, seed=0):
    rng_e = np.random.default_rng(seed)
    X = rng_e.normal(beta_x_mean, 1.0, n)   # mechanism p(X) differs
    Y = beta_xy * X + rng_e.normal(0, noise_scale, n)  # p(Y|X) fixed
    S = 0.5 * Y + rng_e.normal(0, 0.3, n)  # spurious: S correlates with Y via shared cause
    return X, Y, S

n = 500
X1, Y1, S1 = generate_env(n, beta_xy=2.0, beta_x_mean=0.0, seed=1)
X2, Y2, S2 = generate_env(n, beta_xy=2.0, beta_x_mean=2.0, seed=2)   # p(X) shifts

# Model A: use X (causal feature)
# Model B: use S (spurious feature)
def mse(pred, true):
    return ((pred - true)**2).mean()

# Train on env1, test on env2
X_feat1 = X1.reshape(-1,1)
S_feat1 = S1.reshape(-1,1)
model_X = LinearRegression().fit(X_feat1, Y1)
model_S = LinearRegression().fit(S_feat1, Y1)

mse_X_e1 = mse(model_X.predict(X1.reshape(-1,1)), Y1)
mse_S_e1 = mse(model_S.predict(S1.reshape(-1,1)), Y1)
mse_X_e2 = mse(model_X.predict(X2.reshape(-1,1)), Y2)
mse_S_e2 = mse(model_S.predict(S2.reshape(-1,1)), Y2)

print(f"{'':20} {'Env 1 (train)':>15} {'Env 2 (test)':>15}")
print(f"{'Causal (X)':20} {mse_X_e1:>15.4f} {mse_X_e2:>15.4f}")
print(f"{'Spurious (S)':20} {mse_S_e1:>15.4f} {mse_S_e2:>15.4f}")
print(f"\nCausal feature coefficient: {model_X.coef_[0]:.4f}  (true: 2.0)")
print(f"Spurious feature is stable: no, its correlation with Y changes across envs")
```

Reading the four numbers this code prints makes the point and warns against a
common confusion. Raw accuracy on a single environment does not certify a feature
as causal: the spurious feature $S$ posts a *lower* MSE than the causal $X$ in both
environments ($0.35$ vs $1.11$ on env 1, $0.55$ vs $1.03$ on env 2), because $S$
sits downstream of $Y$ and so tracks it tightly in-sample. What separates them is
behaviour under the shift, not the level of the error:

- **Causal feature $X$:** MSE $1.11 \to 1.03$ as $p(X)$ shifts, a change of $-7\%$.
  Flat, because the recovered coefficient ($1.9995$) matches the true mechanism
  $p(Y \mid X)$, which the shift left untouched.
- **Spurious feature $S$:** MSE $0.35 \to 0.55$, a $+59\%$ jump, because the
  $S$-to-$Y$ association is induced by $Y$'s distribution, and shifting $p(X)$
  shifts $p(Y)$ with it.

The lesson is that you cannot rank a causal and a spurious feature by their error on
one environment; you rank them by how that error moves across environments. The
causal feature's error is invariant, which is the property IRM and SMS are built to
detect.

## Exercises

**Exercise 1.** State the IRM objective in words and explain why a causally
invariant feature satisfies its constraint while a spurious correlate does not.
Use the lesson's SCM $X \to Y$ with $p(X)$ shifting across environments and a
spurious $S$ downstream of $Y$.

<details>
<summary>Solution</summary>

**Step 1.** IRM seeks a representation $\Phi$ such that one classifier $w$ is
simultaneously optimal on top of $\Phi$ in every environment $e$, while minimizing
the summed risk. The constraint is that $w$ lies in $\arg\min_w \mathcal{R}_e(w
\circ \Phi)$ for all $e$ at once.

**Step 2 (causal feature).** Predicting $Y$ from $X$ uses the mechanism
$p(Y \mid X)$, which the SMS hypothesis and the simulation hold fixed while only
$p(X)$ shifts. The optimal coefficient on $X$ is the same in every environment, so
$\Phi(x) = x$ satisfies the IRM constraint.

**Step 3 (spurious feature).** $S$ sits downstream of $Y$, so $p(Y \mid S)$ is
induced by $p(Y)$, which moves when $p(X)$ (hence $p(Y)$) shifts. The optimal
coefficient on $S$ differs across environments, so no single $w$ is optimal
everywhere: $S$ violates the constraint. IRM therefore prefers $X$ over $S$ even
though $S$ may have lower in-environment error.

</details>

**Exercise 2.** The sparse mechanism shift hypothesis says a distribution change
across environments alters only a few mechanisms. Explain how this assumption turns
multi-environment data into a lever for causal discovery, and contrast it with what
a single environment can identify.

<details>
<summary>Solution</summary>

**Step 1.** Factor the joint distribution into autonomous mechanisms
$\prod_i p(X_i \mid \mathrm{Pa}(X_i))$ (the ICM principle). Each environment is one
such factorization; under SMS, two environments share all but a sparse subset of
factors.

**Step 2.** Comparing environments localizes the change. The mechanisms that stay
fixed are invariant and so are candidate causal mechanisms; the few that differ
mark where an intervention or shift occurred. Discovery becomes a search for the
sparsest set of differing factors that explains the cross-environment data, which
pins down structure that correlations within one environment cannot.

**Step 3 (single environment).** With one environment there is nothing to compare,
so observationally equivalent SCMs (different graphs with the same joint
distribution) are indistinguishable. Multiple environments break that tie by
exposing which factors are stable, which is exactly the information SMS guarantees
is sparse and therefore usable.

</details>

**Exercise 3 (code).** Using numpy only, reproduce the invariance test from the
lesson. Build two environments from $X \to Y$ ($p(X)$ shifts, $p(Y \mid X)$ fixed)
with a spurious $S$ downstream of $Y$. Fit each single-feature predictor on
environment 1 by least-squares and measure test MSE on environment 2. Assert the
causal feature's MSE is nearly invariant across environments while the spurious
feature's MSE grows.

<details>
<summary>Solution</summary>

```python
import numpy as np

def gen(n, x_mean, seed):
    rng = np.random.default_rng(seed)
    X = rng.normal(x_mean, 1.0, n)
    Y = 2.0 * X + rng.normal(0, 1.0, n)      # p(Y|X) fixed: slope 2
    S = 0.5 * Y + rng.normal(0, 0.3, n)      # spurious, downstream of Y
    return X, Y, S

X1, Y1, S1 = gen(500, 0.0, 1)
X2, Y2, S2 = gen(500, 2.0, 2)                 # p(X) shifts

def fit(feat, y):                             # least-squares with intercept
    A = np.column_stack([np.ones_like(feat), feat])
    coef, *_ = np.linalg.lstsq(A, y, rcond=None)
    return coef

def mse(coef, feat, y):
    A = np.column_stack([np.ones_like(feat), feat])
    return np.mean((A @ coef - y) ** 2)

cX = fit(X1, Y1)
cS = fit(S1, Y1)
mse_X1, mse_X2 = mse(cX, X1, Y1), mse(cX, X2, Y2)
mse_S1, mse_S2 = mse(cS, S1, Y1), mse(cS, S2, Y2)

# Causal feature: error roughly invariant; spurious: error grows under shift
assert abs(mse_X2 - mse_X1) / mse_X1 < 0.2
assert mse_S2 > 1.4 * mse_S1
print(round(mse_X1, 3), round(mse_X2, 3), round(mse_S1, 3), round(mse_S2, 3))
```

The causal predictor reuses the fixed mechanism $p(Y \mid X)$, so its error barely
moves when $p(X)$ shifts. The spurious predictor leans on the $S$-to-$Y$
association, which rides on $p(Y)$ and so degrades once the shift propagates: its
test MSE jumps, flagging it as non-invariant.

</details>

This lesson closes pillar B. The causal inference thread connects forward to
pillar F (probabilistic graphical models and VI/MCMC) and backward to pillar A
(probability, information theory) through the language of conditional independence
and graphical structure.

<Footnotes>
  <FNItem n="1">**Schölkopf, B., Locatello, F., Bauer, S., Ke, N. R., Kalchbrenner, N., Goyal, A., & Bengio, Y.** (2021). "Toward causal representation learning". *Proceedings of the IEEE*, 109(5), 612-634. The survey that frames independent causal mechanisms, the sparse mechanism shift hypothesis, and identifiability of causal factors ([doi:10.1109/JPROC.2021.3058954](https://doi.org/10.1109/JPROC.2021.3058954)).</FNItem>
</Footnotes>
