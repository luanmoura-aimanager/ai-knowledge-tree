...
searching more clusters raises recall and latency<FN n="1" />. In **HNSW** (the small-world graph) it
is `efSearch`, the size of the candidate frontier kept during the greedy traversal:
larger `efSearch` explores more of the graph, finding more true neighbors at more cost<FN n="2" />.
