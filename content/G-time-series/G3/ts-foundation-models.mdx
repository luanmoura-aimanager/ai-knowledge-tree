...
then forecasts a brand-new series **zero-shot**, with no fitting at all. This is the
...
beats specialized forecasters zero-shot. The decoder-only generation loop is the
...
trained baselines. The open issues are specific to time series: covariates and
