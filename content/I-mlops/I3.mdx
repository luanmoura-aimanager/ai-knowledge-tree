...
order: 3
estimatedMinutes: 18
goal: "Lay out the choices that turn a trained model into a service: how requests reach the model, why LLM serving needs batching and a KV cache, and where quantization and distillation buy headroom. Conceptual by design, with no runnable code surface."
prerequisites: ["D7", "E5"]
codeExempt: true
---
...
- **Quantization** stores weights (and sometimes activations) in fewer bits,
  for example 8-bit or 4-bit instead of 16. It cuts memory and bandwidth, which for
